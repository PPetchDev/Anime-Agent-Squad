...
describe("createApiServer", () => {
  ...
  it("POST /api/hooks/user-prompt-submit auto-renames generated default terminal names", async () => {
    ...
    const snapshots = (await listResponse.json()) as Array<Record<string, unknown>>;
    ...
    expect(snapshots[0]).not.toHaveProperty("characterId");
    expect(snapshots[0]).not.toHaveProperty("customAvatarPath");
  });
  ...
  it("persists terminal character identity and includes it in snapshots", async () => {
    const workspaceCwd = mkdtempSync(join(tmpdir(), "octogent-api-test-"));
    temporaryDirectories.push(workspaceCwd);
    const baseUrl = await startServer({
      workspaceCwd,
    ...
    const registryDocument = await waitForRegistryDocument<{
      ...
      document.terminals.some(
        (terminal) =>
          terminal.terminalId === "terminal-1" &&
          terminal.characterId === "ren" &&
          terminal.customAvatarPath === "/custom/ren.png",
      ),
    );
    expect(registryDocument.terminals).toEqual(
      expect.arrayContaining([
