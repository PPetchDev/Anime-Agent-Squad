...
const {
  createShellEnvironmentMock,
  ensureSpawnHelperMock,
  loadWritableNodePtySpawnFallbackMock,
  spawnMock,
} = vi.hoisted(() => ({
  createShellEnvironmentMock: vi.fn(() => ({})),
  ensureSpawnHelperMock: vi.fn(),
  loadWritableNodePtySpawnFallbackMock: vi.fn<() => unknown>(() => undefined),
  spawnMock: vi.fn(),
}));
...
vi.mock("../src/terminalRuntime/ptyEnvironment", () => ({
  ...
  loadWritableNodePtySpawnFallback: loadWritableNodePtySpawnFallbackMock,
}));
...
describe("createSessionRuntime", () => {
  ...
  beforeEach(() => {
    ...
    loadWritableNodePtySpawnFallbackMock.mockClear();
    spawnMock.mockReset();
  });
  ...
  it("retries PTY startup with a writable node-pty fallback when the package helper is not executable", () => {
    const tentacleId = "tentacle-1";
    const terminals = new Map<string, PersistedTerminal>([
      [
        tentacleId,
    ...
    const websocketServer = new FakeWebSocketServer();
    const pty = new FakePty();
    const fallbackSpawn = vi.fn(() => pty);
    const transcriptDirectoryPath = createTemporaryDirectory();
    spawnMock.mockImplementationOnce(() => {
      throw new Error("posix_spawnp failed.");
    });
    loadWritableNodePtySpawnFallbackMock.mockReturnValueOnce(fallbackSpawn);
    ...
    expect(sessions.get(tentacleId)?.pty).toBe(pty);
