...
import {
  ...
  loadWritableNodePtySpawnFallback,
} from "./ptyEnvironment";
...
type CreateSessionRuntimeOptions = {
  ...
  loadFallbackNodePtySpawn?: () => typeof spawn | undefined;
};
...
export const createSessionRuntime = ({
  ...
  onSessionStart,
  onSessionEnd,
  loadFallbackNodePtySpawn = loadWritableNodePtySpawnFallback,
  ...
  const ensureSession = (sessionId: string, tentacleId: string) => {
    ...
    try {
      ...
      const fallbackSpawn = loadFallbackNodePtySpawn();
      if (!fallbackSpawn) {
        throw new Error(
      ...
      try {
        pty = fallbackSpawn(shellLaunch.command, shellLaunch.args, {
          cols: DEFAULT_PTY_COLS,
          rows: DEFAULT_PTY_ROWS,
          cwd: tentacleCwd,
          env: createShellEnvironment({ octogentSessionId: sessionId }),
        ...
        throw new Error(
          `Unable to start terminal shell (${shellLaunch.command}): ${toErrorMessage(error)}; fallback failed: ${toErrorMessage(fallbackError)}`,
        );
      }
    }
