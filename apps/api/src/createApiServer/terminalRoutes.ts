...
import { prependCharacterPrompt } from "../terminalRuntime/characterPrompts";
...
export const handleTerminalsCollectionRoute: ApiRouteHandler = async (
  ...
  const characterResult = parseTerminalCharacterIdentity(bodyReadResult.payload);
  ...
  try {
    ...
    if (characterResult.characterId !== undefined) {
      createTerminalInput.characterId = characterResult.characterId;
    }
    if (characterResult.customAvatarPath !== undefined) {
      createTerminalInput.customAvatarPath = characterResult.customAvatarPath;
    }
    ...
    const characterInitialPrompt = prependCharacterPrompt(
      createTerminalInput.initialPrompt,
      createTerminalInput.characterId,
    );
    if (characterInitialPrompt !== undefined) {
      createTerminalInput.initialPrompt = characterInitialPrompt;
    }

    const characterInitialInputDraft = prependCharacterPrompt(
      createTerminalInput.initialInputDraft,
      createTerminalInput.characterId,
    );
    if (characterInitialInputDraft !== undefined) {
      createTerminalInput.initialInputDraft = characterInitialInputDraft;
    }
