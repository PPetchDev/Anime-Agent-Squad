import { isBuiltInCharacterId } from "@octogent/core";
...
export const parseTerminalCharacterIdentity = (payload: unknown) => {
  if (payload === null || payload === undefined) {
    return {
      characterId: undefined as string | undefined,
      customAvatarPath: undefined as string | undefined,
      error: null as string | null,
    };
  }
  ...
  const rawCharacterId = record.characterId;
  const rawCustomAvatarPath = record.customAvatarPath;
  const characterId =
    typeof rawCharacterId === "string" && rawCharacterId.trim().length > 0
      ? rawCharacterId.trim()
      : undefined;
  const customAvatarPath =
    typeof rawCustomAvatarPath === "string" && rawCustomAvatarPath.trim().length > 0
      ? rawCustomAvatarPath.trim()
      : undefined;
  ...
  if (rawCharacterId !== undefined && typeof rawCharacterId !== "string") {
    return {
      characterId: undefined as string | undefined,
      customAvatarPath: undefined as string | undefined,
      error: "Terminal characterId must be a string.",
    };
  }

  if (characterId && !isBuiltInCharacterId(characterId)) {
    return {
      characterId: undefined as string | undefined,
      customAvatarPath: undefined as string | undefined,
      error: "Unknown terminal characterId.",
    };
  }

  if (rawCustomAvatarPath !== undefined && typeof rawCustomAvatarPath !== "string") {
    return {
      characterId: undefined as string | undefined,
      customAvatarPath: undefined as string | undefined,
      error: "Terminal customAvatarPath must be a string.",
    };
  }
