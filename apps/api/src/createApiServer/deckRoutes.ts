...
export const handleDeckTodoSolveRoute: ApiRouteHandler = async (
  ...
  try {
    const workerPrompt = prependCharacterPrompt(
      await buildSingleTodoWorkerPrompt({
        promptsDir,
        workspaceCwd,
        tentacleId,
        tentacleName,
...
export const handleDeckTentacleSwarmRoute: ApiRouteHandler = async (
  ...
  try {
    if (!needsParent) {
      ...
      const workerPrompt = prependCharacterPrompt(
        await resolvePrompt(promptsDir, "swarm-worker", {
          tentacleName,
          tentacleId,
          tentacleContextPath,
    ...
    if (needsParent && parentTerminalId) {
      ...
      const workerSpawnCommands = targetItems
        .map((item) => {
          ...
          if (characterResult.customAvatarPath) {
            commandParts.push(
              `--custom-avatar-path ${shellSingleQuote(characterResult.customAvatarPath)}`,
            );
          }
      ...
      const parentPrompt = prependCharacterPrompt(
        await resolvePrompt(promptsDir, "swarm-parent", {
          tentacleName,
          tentacleId,
