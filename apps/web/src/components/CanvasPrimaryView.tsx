...
import type {
  CreateTerminalCharacterOptions,
  PendingDeleteTerminal,
} from "../app/hooks/useTerminalMutations";
import {
...
type CanvasPrimaryViewProps = {
  ...
  onCreateAgent?:
    | ((
        tentacleId: string,
        character?: CreateTerminalCharacterOptions,
...
export const CanvasPrimaryView = ({
  ...
  const handleCreateAgent = useCallback(
    (tentacleId: string, character?: CreateTerminalCharacterOptions) => {
      if (!onCreateAgent) return;
      setContextMenu(null);
      const result = onCreateAgent(tentacleId, character);
            ...
            {Array.from(openTentacles.entries()).map(([nodeId, node]) => (
              <CanvasTentaclePanel
                ...
                onCreateAgent={(tentacleId, character) => {
                  handleCreateAgent(tentacleId, character);
                }}
